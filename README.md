...
API-запросы выполняются параллельно (флаг `-c`, по умолчанию 5).

При подключении выводится [баннер роутера](#info--информация-о-роутере).
...
| `--concurrency` | `-c` | Параллельных запросов к API (по умолчанию 5, 0 = последовательно) |
...
С флагом `--entries` показывает все записи конкретного списка (адрес, комментарий, статус).
...
| `--user` | `-u` | Имя пользователя API |
| `--pass` | `-p` | Пароль |
| `--entries` | `-e` | Показать все записи указанного списка |
| `--insecure` | `-k` | Не проверять TLS сертификат |

```bash
# список всех address-list с количеством записей
./mikrotik-lists-manager list -H 192.168.1.1 -u admin
...
Подключается к роутеру и выводит информационный блок: модель платы, версию RouterOS, архитектуру, CPU, память, аптайм. Для RouterBoard-устройств дополнительно: модель, ревизию, серийный номер, версию прошивки (с предупреждением если доступно обновление) и версию заводской прошивки.

Тот же баннер автоматически отображается в начале команд `sync`, `append` и `remove`.
