...
- [Команды](#команды)
  ...
  - [purge](#purge--полная-очистка-списка-на-роутере)
  - [export](#export--экспорт)
  - [convert](#convert--оффлайн-конвертация-форматов)
...
Установка конкретной версии (например, `v1.7.0`):

```bash
go install github.com/D4n13l3k00/mikrotik-lists-manager/cmd/mlm@v1.7.0
```
...
Для многотысячных списков используйте **турбо-режим** `--fast` (или синоним `--batch`): изменения группируются в пакеты (по умолчанию по 250) и применяются атомарно через CLI-скрипты RouterOS за секунды вместо десятков минут по REST API.

**Безопасный перехват Ctrl+C**: при прерывании синхронизации пользователем утилита не выводит мусорных ошибок REST API, завершается с чистым кодом 130 и сразу предлагает выполнить автоматический откат (`Rollback`) списка к исходному снимку (если список изначально был пуст — откат выполняется мгновенно одной командой за 0.1 с).
...
| `--fast` | | **Турбо-режим**: применение изменений пакетами через RouterOS скрипты (в десятки раз быстрее REST API) |
| `--batch` | | Синоним `--fast`: пакетное применение |
| `--batch-size` | | Размер пакета записей для `--fast` (по умолчанию 250) |
...
# турбо-синхронизация списков из тысяч записей (пакетами через RouterOS скрипты)
mlm sync large.list -H 192.168.1.1 -u admin -l vpn-routes --fast
...
| `--format` | `-f` | Формат файла: `auto`, `native`, `mikrotik` |
| `--dry-run` | `-n` | Показать изменения без применения |
| `--fast` | | **Турбо-режим**: пакетное добавление через RouterOS скрипты |
| `--batch` | | Синоним `--fast`: пакетное добавление |
| `--batch-size` | | Размер пакета записей (по умолчанию 250) |
| `--resolve-domains` | | Разрешать доменные имена в IP-адреса через DNS |
| `--dns` | | Пользовательский DNS-сервер для резолвинга |
| `--concurrency` | `-c` | Число параллельных запросов к API (по умолчанию 5, 0 = последовательно) |
| `--insecure` | `-k` | Не проверять TLS сертификат |
...
# турбо-добавление большого файла пакетами
...
| `--format` | `-f` | Формат файла: `auto`, `native`, `mikrotik` |
| `--dry-run` | `-n` | Показать изменения без применения |
| `--fast` | | **Турбо-режим**: пакетное удаление через RouterOS скрипты |
| `--batch` | | Синоним `--fast`: пакетное удаление |
| `--batch-size` | | Размер пакета записей (по умолчанию 250) |
| `--concurrency` | `-c` | Число параллельных запросов к API (по умолчанию 5, 0 = последовательно) |
| `--insecure` | `-k` | Не проверять TLS сертификат |
...
# турбо-удаление большого набора записей
...
Для защиты от случайного сноса данных и максимальной скорости:
...
3. **Безопасная симуляция**: флаг `-n / --dry-run` позволяет проверить количество записей без отправки запросов на удаление.
4. **Мгновенное удаление**: выполняется атомарно на стороне роутера через CLI-скрипт (`/ip firewall address-list remove [find where list="..."]`) за долю секунды даже для списков из десятков тысяч записей. При отсутствии прав на выполнение скриптов у пользователя API происходит автоматический fallback на параллельное удаление отдельных записей.
