...
Обновление до последней версии выполняется аналогичной командой:

```bash
go install github.com/D4n13l3k00/mikrotik-lists-manager/cmd/mlm@latest
```

Установка конкретной версии (например, `v1.6.1`):

```bash
go install github.com/D4n13l3k00/mikrotik-lists-manager/cmd/mlm@v1.6.1
```

Бинарник `mlm` устанавливается (или обновляется) в `$GOPATH/bin` (по умолчанию `~/go/bin` на Linux/macOS или `%USERPROFILE%\go\bin` на Windows), который должен находиться в системной переменной `PATH`.
...
# сконвертировать список оффлайн в команды MikroTik (.rsc)
mlm convert blocked.list -o blocked.rsc

# полностью очистить список на роутере (с автосохранением снимка)
mlm purge -l vpn-routes -H 192.168.1.1 -u admin
```
...
### `purge` — полная очистка списка на роутере

Удаляет **все** записи из указанного address-list (или всех списков) на роутере MikroTik.

Для защиты от случайного сноса данных:
1. **Автоматический снимок**: перед удалением создаётся снимок списка (`snapshot`), позволяющий в любой момент восстановить все данные командой `mlm rollback -l <list>`.
2. **Запрос подтверждения**: в интерактивном терминале запрашивается подтверждение с указанием количества удаляемых записей (пропускается флагом `-y / --force`). В скриптах без флага `--force` команда завершается с ошибкой безопасности.
3. **Безопасная симуляция**: флаг `-n / --dry-run` позволяет проверить количество записей без отправки DELETE-запросов.
4. **Параллелизм**: записи удаляются параллельно через пул воркеров (флаг `-c`, по умолчанию 5) с отображением прогресс-бара.
...
| `--pass` | `-p` | Пароль |
| `--list` | `-l` | Имя address-list, можно несколько |
| `--all` | | Очистить ВСЕ списки на роутере |
| `--dry-run` | `-n` | Показать изменения без применения |
| `--force` | `-y` | Не запрашивать подтверждение перед очисткой |
| `--no-snapshot` | | Не создавать автоматический снимок перед очисткой |
| `--verbose` | `-v` | Выводить каждую запись даже при прогресс-баре |
| `--concurrency` | `-c` | Число параллельных запросов к API (по умолчанию 5, 0 = последовательно) |
| `--insecure` | `-k` | Не проверять TLS сертификат |

```bash
# очистить список с интерактивным подтверждением
mlm purge -H 192.168.1.1 -u admin -l vpn-routes

# симуляция очистки (dry-run)
mlm purge -H 192.168.1.1 -u admin -l vpn-routes -n

# принудительная очистка без подтверждения
mlm purge -H 192.168.1.1 -u admin -l vpn-routes -y

# очистка нескольких списков
mlm purge -H 192.168.1.1 -u admin -l vpn,blocked -y

# очистить ВСЕ списки на роутере
mlm purge -H 192.168.1.1 -u admin --all -y
```
...
### `convert` — оффлайн-конвертация форматов

Конвертирует список адресов между текстовым форматом `native` (`.list`/`.lst`) и командами RouterOS export (`.rsc`) локально без подключения к роутеру.

Поддерживает источники: локальные файлы, прямые HTTP/HTTPS ссылки и стандартный ввод (stdin `-`).
При выводе в stdout служебные логи отправляются в `stderr`, что позволяет использовать команду в конвейерах (pipes).
...
```shell
mlm convert [файл|URL|-] [флаги]
```

| Флаг | Короткий | Описание |
|------|----------|----------|
| `--format` | `-f` | Целевой формат: `auto` (по умолчанию), `native`, `mikrotik` |
| `--list` | `-l` | Имя address-list для формата mikrotik (по умолчанию: имя файла без расширения) |
| `--output` | `-o` | Записать результат в файл вместо stdout |
| `--write` | `-w` | Перезаписать исходный файл результатом (только для локальных файлов) |
| `--resolve-domains` | | Разрешать доменные имена в IP-адреса через DNS |
| `--dns` | | Пользовательский DNS-сервер для резолвинга (например: `1.1.1.1:53`) |

```bash
# автоопределение: native -> mikrotik (.rsc в stdout)
mlm convert blocked.list

# конвертация native в файл .rsc с указанием имени списка
mlm convert blocked.list -l vpn-routes -o blocked.rsc

# конвертация mikrotik export (.rsc) обратно в native (.list)
mlm convert export.rsc -o export.list

# чтение из stdin и вывод в pipe
cat list.txt | mlm convert - -l mylist | ssh admin@192.168.1.1

# конвертация удаленного списка напрямую по URL
mlm convert https://example.com/ips.txt -l cloud -o cloud.rsc

# резолвинг доменов в IP перед конвертацией в mikrotik
mlm convert domains.list --resolve-domains -o resolved.rsc

# перезапись файла результатом на месте (in-place)
mlm convert list.rsc -w
```
